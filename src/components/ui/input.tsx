...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  const isNativeDateTime = type ? nativeDateTypes.has(type) : false
  ...
  return (
    <InputPrimitive
      ...
      className={cn(
        ...
        isNativeDateTime &&
          "block appearance-none [&::-webkit-date-and-time-value]:min-w-0 [&::-webkit-date-and-time-value]:text-left [&::-webkit-datetime-edit]:min-w-0 [&::-webkit-datetime-edit-fields-wrapper]:min-w-0",
        className
      )}
      {...props}
    />
